	...
	float YawOverride)
{
	...
	{
		// This means that ArrayCopy contains all elements that we want or better yet has all the Number of Points that we specify 
		...
		// We only care about the Z location because we need to find the elevation on where to spawn the shards
		const FVector AdjustedLocation = FVector(Pt->GetComponentLocation().X, Pt->GetComponentLocation().Y, HitResult.ImpactPoint.Z);
		Pt->SetWorldLocation(AdjustedLocation);
		// ImpactNormal is the vector from the ground straight Out perpendicular we call it orthogonal.
