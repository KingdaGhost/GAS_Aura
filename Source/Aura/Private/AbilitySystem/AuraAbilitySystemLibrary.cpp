...
{
	// This is for giving common abilities like hit react and death mostly for montages
	UCharacterClassInfo* CharacterClassInfo = GetCharacterClassInfo(WorldContextObject);
	...
	// This is for giving ability that is specific to each Character Class
...
FGameplayTag UAuraAbilitySystemLibrary::GetDamageType(const FGameplayEffectContextHandle& EffectContextHandle)
{
	if (const FAuraGameplayEffectContext* AuraEffectContext = static_cast<const FAuraGameplayEffectContext*>(EffectContextHandle.Get()))
	{
		...
		{
			// We need to dereference the DamageType as it is a TSharedPtr.
	...
	const FGameplayTag& InDamageType)
{
	if (FAuraGameplayEffectContext* AuraEffectContext = static_cast<FAuraGameplayEffectContext*>(EffectContextHandle.Get()))
    {
		// We need to create a TSharedPtr to be able to pass into the function SetDamageType since the member variable is of type TSharedPtr
...
void UAuraAbilitySystemLibrary::SetKnockbackForce(FGameplayEffectContextHandle& EffectContextHandle, const FVector& InKnockback)
{
	if (FAuraGameplayEffectContext* AuraEffectContext = static_cast<FAuraGameplayEffectContext*>(EffectContextHandle.Get()))
	{
	...
	const FVector& InOrigin)
{
	if (FAuraGameplayEffectContext* AuraGameplayEffectContext = static_cast<FAuraGameplayEffectContext*>(EffectContextHandle.Get()))
	{
		AuraGameplayEffectContext->SetRadialDamageOrigin(InOrigin);
	}
}

FGameplayEffectContextHandle UAuraAbilitySystemLibrary::ApplyDamageEffect(const FDamageEffectParams& DamageEffectParams)
{
	if (DamageEffectParams.SourceAbilitySystemComponent == nullptr && DamageEffectParams.TargetAbilitySystemComponent == nullptr) return FGameplayEffectContextHandle();
...
{	//The below lines is taken from GameplayStatics::ApplyRadialDamageWithFallOff
	...
	if (const UWorld* World = GEngine->GetWorldFromContextObject(WorldContextObject, EGetWorldErrorMode::LogAndReturnNull))
	{
		...
		{
			// Checks if Actor Implements Interface and Check if it is alive
	...
	TArray<AActor*>& OutClosestTargets, const FVector& Origin)
{
	...
	// Creating a copy of Actors so we can remove already found Closest Actor in the loop while checking
	TArray<AActor*> ActorsToCheck = Actors;
	int32 NumTargetsFound = 0;

	// As long as the Number of Targets found is less than Max Targets then we go through the loop
	while (NumTargetsFound < MaxTargets)
	{
		if (ActorsToCheck.Num() == 0) break;

		// Max possible limit that a double can be. It is the largest double number
		double ClosestDistance = TNumericLimits<double>::Max();
		AActor* ClosestActor;
		// Find the single Actor that is closest to the origin and then add it to the OutClosestTargets and Remove it from ActorsToCheck
